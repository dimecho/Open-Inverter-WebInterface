...
class phpSerial
{
	...
	function deviceSet ($device)
	{
		if ($this->_dState !== SERIAL_DEVICE_OPENED)
		{
			...
			{
				if (preg_match("@^COM(\d+):?$@i", $device, $matches) and $this->_exec(exec("mode " . $device . " xon=on BAUD=115200")) === 0)
				{
					$this->_windevice = "COM" . $matches[1];
					$this->_device = "\\.\com" . $matches[1];
					$this->_dState = SERIAL_DEVICE_SET;
					$this->_modeparams = $this->_windevice;
					return true;
				}
			}
	...
	function deviceOpen ($mode = "r+b")
	{
		...
		if ($this->_os === "windows")
		{
			$this->_exec("mode $this->_modeparams", $out);
			$this->_dHandle = dio_open($this->_device, O_RDWR);
		}
		else
		{
	...
	function deviceClose ()
	{
		...
		if ($this->_os === "windows")
		{
			dio_close($this->_dHandle);	//Has no return value
			$this->_dHandle = null;
			$this->_dState = SERIAL_DEVICE_SET;
			return true;
		}
	...
	function confBaudRate ($rate)
	{
		...
		if (isset($validBauds[$rate]))
		{
            ...
            {
                $ret = 0;
				$this->_modeparams .= " BAUD=".$validBauds[$rate];
            }
            else return false;
	...
	function confParity ($parity)
	{
		...
		{
			$ret = 0;
			$this->_modeparams .= " PARITY=" . $parity{0};
		}
	...
	* Turns the echo off to remote
	...
	function confFlowControl ($mode)
	{
		...
		{
			$ret = 0;
			$this->_modeparams .= " ".$windowsModes[$mode];
		}
	...
	function readPort ($count = 0)
	{
		...
		elseif ($this->_os === "windows")
		{
			...
			if ($count !== 0)
			{
				$content = dio_read($this->_dHandle, $count);
			}
			else
			{ //Well yes... 64k limit here
				$content = dio_read($this->_dHandle, 65536);
			}
	...
	function serialflush ()
	{
		...
		if ($this->_os === "windows" && dio_write($this->_dHandle, $this->_buffer))
		{
			$this->_buffer = "";
			return true;
		}
		else if (fwrite($this->_dHandle, $this->_buffer) !== false)
		{
			$this->_buffer = "";
			return true;
		}
