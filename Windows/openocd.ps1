if($args[0] -eq "uninstall") {
    ...
    if (-Not (Test-Path $openocd)){
        ...
        Start-Process "openocd.exe" -ArgumentList "-f ./scripts/$1 -f ./scripts/board/olimex_stm32_h103.cfg  -c ""init"" -c ""reset init"" -c ""halt"" -c ""flash write_image erase $0 0x08000000"" -c ""reset"" -c ""shutdown"""
    }
}